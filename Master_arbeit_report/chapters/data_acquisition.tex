...
\subsection{Robot guided trajectory} Although robot-guided data set is available as a part of the pipeline, we will briefly look at the important aspects of it. As shown in the \cref{fig:robotic_data_acquisition}  setup consists of 2 cameras (RGB-D and a tracking camera), a robot, phantom head with EEG cap. Kinect is mounted directly on end-effector of the robot while tracking camera is firmly mounted to the ceiling (Cambar B2 is shown in the graphic instead of fusionTrac 500). The position of tracking camera from robot base $\tfMat{Base}{T}{tCam}$ is determined using hand-eye calibration. Since kinect is mounted directly on the end-effector, a variant form of hand-eye calibration known as eye-in-hand calibration is performed to calculate the position of Kinect from robot base $\tfMat{Base}{T}{kCam}$. First, all the electrodes are digitized using a reflective marker and fusionTrac. Then the robot is precisely guided around the phantom head on a circular trajectory while continuously capturing RGB and depth images at 30fps. The process is repeated several times with same circular trajectory each time with re-positioned phantom head. 
...
The Cambar B2 communication server,  pose tracking software and ROS, kinect software were on two separate computers. Cambar B2 software provides a time stamp in milliseconds relative to starting time of the camera (doesn't use system time as basis) with alternating 62.5 and 66.66 fps while kinect data is recorded at 30 fps with ROS timestamps as shown in \cref{fig:time_sync}. Although data acquisition on both computers were manually started together, we expect time delay from pressing start button to actual starting. As both cameras doesn't have a common  time as reference, we cannot assume first entry of both cameras be corresponds to each other. Therefore, time synchronization is inevitable. First task is to determine which Cambar entry corresponds to first entry of Kinect. This may prompt to skip first few entries from the Cambar data. Once we found the correct entry, we express all the other time stamps in relation to just found entry. Then, at time \textit{t}, we match the Kinect time stamps with Cambar. If Kinect pose recorded in Cambar at exact time is not available, we pick the pose closest to time \textit{t}. In order to choose the correct first entry in Cambar data, we first map the point clouds using kinect poses and iterate over first 60 frames (assuming time delay is not more than 1 second), at each frame, we measure the cluster inertia \cref{eq:cluster_center_inertia} and pick the entry corresponds to least inertia. Since change in kinect raw depth value also has an effect on cluster inertia, later we perform two optimization together (details to follow). The \cref{fig:cs_301_red_14_time_sync} shows an extreme case where first 48 frames had to be skipped to achieve desired goal.
